  ...
  return (
    <div className="p-6 lg:p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-start gap-4">
        <Link to="/disciplinas"
          className="mt-1 p-2 rounded-xl text-slate-400 hover:text-white hover:bg-slate-800/50 transition">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-1">
            <div className="p-2.5 rounded-xl" style={{ backgroundColor: topicColor + '22', color: topicColor }}>
            ...
            <div>
              ...
              <h1 className="text-2xl font-bold text-white mt-1">{topic.nome}</h1>
            </div>
          </div>
          <p className="text-slate-400 text-sm mt-1 ml-14">{topic.descricao}</p>
        </div>
      </div>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <div className="relative flex-1 sm:w-64">
            ...
            <input type="text" placeholder="Buscar anotações..."
              ...
              className="w-full pl-10 pr-4 py-2 rounded-xl bg-slate-800/50 border border-slate-700/50 text-white placeholder-slate-500 text-sm outline-none focus:ring-2 focus:ring-indigo-500 transition" />
          </div>
          <button onClick={() => { setEditingNote(null); setShowEditor(true); }}
            className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white rounded-xl transition whitespace-nowrap cursor-pointer"
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16">
          ...
          {!search && (
            <p className="text-slate-500 text-sm">Clique em "Nova Anotação" para começar.</p>
          )}
        </div>
      ) : (
